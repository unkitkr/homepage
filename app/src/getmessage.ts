...
class commandProcessor {
  ...
  private subscribersDb = this.db.table(environmentVariables.API_KEY_SUBSCRIBERS_DB!);
  private availableDb = this.db.table(environmentVariables.API_KEY_AVAILABLE_DB!);
  ...
  private subscribeUser = async () => {
    ...
    if (alreadySubscribed && alreadySubscribed.length > 0) {
      this.sendMessage("sendMessage", {
        text: `Hey ${capitalize(this.processedData.firstName)} you're already subscribed 😄`,
        reply_to_message_id: this.processedData.messageId,
        chat_id: this.processedData.chatId,
      });
    } else {
      ...
      if (subscribedUser && subscribedUser.length > 0) {
        this.sendMessage("sendMessage", {
          text: `Thanks for subscribing ${capitalize(this.processedData.firstName)}`,
          reply_to_message_id: this.processedData.messageId,
          chat_id: this.processedData.chatId,
        });
      } else {
        this.sendMessage("sendMessage", {
          text: `Hey ${capitalize(this.processedData.firstName)} we encountered an error on our side`,
          reply_to_message_id: this.processedData.messageId,
          chat_id: this.processedData.chatId,
        });
      }
  ...
  private unsubscribeUser = async () => {
    ...
    if (!userExist) {
      this.sendMessage("sendMessage", {
        text: `Hey ${capitalize(this.processedData.firstName)} you're not subscribed 😄`,
        reply_to_message_id: this.processedData.messageId,
        chat_id: this.processedData.chatId,
      });
    } else {
...
const bodyParser = (obj: string) => {
  const parsed: IMessageBody = JSON.parse(obj);
